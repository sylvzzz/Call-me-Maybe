...
def generate_string_value(model: Small_LLM_Model, quote_token_id: int,
    ...
    """
    Built a list of "safe" tokens once every token that,
    written as plain text, doesn't contain a " character.
    This matters because a stray " showing up in the middle of the value
    would prematurely close the JSON string and corrupt the output.

    Manually added the quote token back in even though
    it technically "contains a quote" (it IS the quote), i allow it,
    because it's the one special token you want the model to be able to choose,
    specifically as the signal "I'm done writing the value."
    Looped, one token at a time:

    Asked the model for logits (its raw opinion on what token comes next,
    given everything written so far prompt +
    whatever's been generated in this loop already).
    Looked only at the scores for the "safe" tokens from step 1/2
    (ignoring all other tokens' scores entirely).

    Picked whichever safe token scored highest i.e.,
    what the model actually thinks is the
    best next character/word-piece, restricted to the safe set.
    If that pick happened to be the quote token stop, the value is finished.
    Otherwise remember that token as part of the answer,
    and loop again, now with one more token added to the context.


    Returned the list of tokens it built up
    """
...
def generate_number_value(model: Small_LLM_Model, input_ids_so_far: list[int],
    ...
    while True:
        ...
        # keep only candidates that are actually grounded in the prompt
        legal_ids = [
